...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "   - ចែករំលែកទីតាំង (Location) របស់អ្នក ដើម្បីទទួលបានព័ត៌មានអាកាសធាតុ និងដីជាក់ស្តែង។\n\n"
        ...
        "<b>Key Features:</b>\n"
        "• <b>Crop Disease Photo Diagnosis:</b> Send a photo of a diseased leaf or plant.\n"
...
async def handle_user_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. Factual soil and weather data
    if telegram_id in USER_LOCATIONS:
        lat, lon = USER_LOCATIONS[telegram_id]
        soil = get_soil_data_with_fallback(lat, lon)
        weather = get_weather_history(lat, lon)
        region_desc = f"Custom Plot Lat: {lat}, Lon: {lon}"
    else:
        lat, lon = 11.5564, 104.9282
        soil = get_soil_data_with_fallback(lat, lon)
        weather = get_weather_history(lat, lon)
        region_desc = f"Default Baseline Lat: {lat}, Lon: {lon}"
    ...
    3. Language & Output Protocol:
       ...
       - Output Format: Provide ONLY the final response to the user. Use clear bullet points and bold section titles. Do NOT display intermediate reasoning or notes.
